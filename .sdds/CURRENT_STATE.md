...
- **BUG CRÍTICO DE SEGURANÇA — RLS bloqueava leitura/escrita durante impersonação mesmo com `workspaceId` correto — RESOLVIDO 2026-06-10 (branch dev, não commitado)**: segunda metade de ADR-004 (ver addendum) — `getWorkspaceContext`/`getCurrentWorkspaceId` resolviam `workspaceId = <impersonado>` (app layer, ADR-004), mas Server Actions de `contacts/` continuavam com `createClient()` RLS-bound ao `auth.uid()` real (superadmin), e `my_workspace_ids()` só retorna PyTec para o superadmin. Resultado: `ContactSourcesSheet` não mostrava as 11 origens da Lekazis (SELECT filtrado por RLS) e a reimportação da planilha Lekazis seria bloqueada (INSERT rejeitado por `WITH CHECK`). Corrigido com novo `getScopedSupabaseClient()` em `src/lib/guards.ts` (retorna `createAdminClient()` durante impersonação validada, senão `createClient()` — drop-in `Promise<SupabaseClient>`), aplicado em `contacts/{actions,import-actions,niche-profile-actions}.ts` (18 call sites). Também: bloco "Criar nova origem" do `ImportContactsDialog` extraído para componente `CreateSourceInline`, agora sempre visível nas etapas `preview` e `result` (antes só existia dentro do alerta de coluna ausente e sumia durante `isSubmitting`). `tsc --noEmit` limpo; `vitest run src/tests/security src/tests/tenant-isolation` → 326/326 (era 323, +3 novos `getScopedSupabaseClient`; `actions-auth.test.ts` ajustado para mockar o novo helper). Ver `discoveries/2026-06-10-impersonation-rls-blocks-data-access.md`. **Pendente**: ~15 arquivos fora de `contacts/` com o mesmo gap (R-009); usuário validar manualmente (origens Lekazis visíveis, criar origem no import, reimportar planilha).

## Próximas ações disponíveis

| Ação | Módulo SDDS | Status |
|---|---|---|
| Validar manualmente impersonando Lekazis (origens visíveis, criar origem no import, reimportar planilha) | — | Ação do usuário |
| Aplicar `getScopedSupabaseClient()` nos ~15 arquivos do R-009 (kanban, settings, auto-parts, auto-sales, fashion, dashboard, area-tracking) | — | PRÓXIMO |
| Spec módulo WA Integrations | 02_CREATE_MODULE_SPEC | Aguarda |
