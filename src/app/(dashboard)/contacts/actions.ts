...
import { getWorkspaceContext, getCurrentWorkspaceId, getScopedSupabaseClient } from "@/lib/guards";
...
export async function createContact(_: unknown, formData: FormData) {
  ...
  const supabaseForWsLookup = await getScopedSupabaseClient();
...
export async function getContactsPageContext() {
  ...
  const [role, supabase] = await Promise.all([getUserRole(workspaceId), getScopedSupabaseClient()]);
