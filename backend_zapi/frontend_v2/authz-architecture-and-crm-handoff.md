...
>
> **Ponto em aberto a confirmar na implementação — tamanho do token.** Se um
> usuário com muitos workspaces/permissões fizer o `perms` resolvido inchar o
> JWT além do limite prático, cai-se no **plano B** (mandar só `role` por
> workspace + mapa `role→perms` versionado embutido nos integradores — ver
> regra de `perms` abaixo). Essa troca é um **bump de `v` coordenado**, não uma
> mudança silenciosa. Até lá, o default é `perms` resolvido.
