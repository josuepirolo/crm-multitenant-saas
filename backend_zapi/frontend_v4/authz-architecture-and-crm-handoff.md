...
> `.sdds/decisions/ADR-007-autorizacao-cross-service-claims-jwt.md` (o lado WA
> já tem o par: `api-zapi-multitenant/.sdds/decisions/ADR-008-...`). Gerado em
> 2026-06-13.

## ✅ Decisão fechada pelo front (2026-06-13)

**Gramática da chave de permissão — RESOLVIDA.** O front confirmou: `grep
'.split(":")'` em `src/` = 0 ocorrências e `getWorkspaceContext` compara a chave
inteira (`permKeys.includes(\`${module}:${action}\`)`). Chaves com dois `:`
(`integration.whatsapp.instance:manage`) podem ser semeadas sem ajuste de código.
Histórico da decisão abaixo.
...
## 3. Contrato do claim `authz` (interface compartilhada — 🔒 **CONGELADO** em 2026-06-13)

> **Status: CONGELADO (v1) como referência de implementação.** Acordado entre
> CRM e WA em 2026-06-13 como o formato a partir do qual os dois times constroem
> em paralelo. Qualquer mudança neste formato exige **bump de `v`** e acordo
> coordenado documentado nesta seção.
>
> ⚠️ **Não é um sign-off final do front sobre o JSON.** O front resolveu a
> decisão bloqueante (gramática da chave — ver topo do doc) e fica de
> **confirmar o formato na implementação, avisando o WA se algo mudar**.
>
> ✅ **Tamanho do token — RESOLVIDO na implementação (2026-06-13).** O CRM
> decidiu carregar em `perms` **apenas as chaves do namespace `integration.*`**
> (não todas as permissões do workspace). Isso mantém o token compacto
> proativamente, sem precisar do plano B. O formato do JSON é idêntico ao §3 —
> mudou só o **conteúdo** de `perms` (subconjunto filtrado). Continua **v1**: o
> WA só faz `perms.includes("integration.whatsapp.*")`, então nada quebra.
> Confirmado OK pelo WA. (Plano B histórico — `role` + mapa embutido — fica como
> fallback caso só o `integration.*` ainda assim inche; seria bump de `v`.)
  ...
  "https://lekazis.app/authz": {
    ...
    "workspaces": {
      "<workspace_id>": {
        ...
        "perms": [                   // chaves EFETIVAS do namespace integration.* (filtradas; ver regra abaixo)
...
- `perms` é a **lista efetiva já resolvida** (granular do banco quando
  `workspace_role_id` definido; senão a matriz hardcoded), **filtrada ao
  namespace `integration.*`** (decisão de implementação 2026-06-13 — mantém o
  token compacto; permissões de outros domínios não vão no claim). O integrador
  faz só `perms.includes("integration.whatsapp.instance:manage")` — não
  re-resolve nada. Consumidor que precise de permissão fora de `integration.*`
  **não deve** assumir que está aqui.
- Manter `perms` **compacto** (só chaves string). O filtro `integration.*` já
  endereça isso; se ainda assim inchar, fallback histórico = mandar só `role`
  por workspace + mapa role→perms versionado embutido (seria **bump de `v`**).
...
- Criar `.sdds/decisions/ADR-007-autorizacao-cross-service-claims-jwt.md`
