...
Implementado com mais profundidade: autenticação (incl. Turnstile com reset automático após falha, mensagens específicas por tipo de erro), dashboard com métricas e gráficos, **módulo de contatos** (incl. carteira de clientes: assigned_to por responsável, compartilhamento pontual via contact_access, RLS por portfolio, origem obrigatória, gerenciamento de origens, criação de nova origem disponível em qualquer etapa do fluxo de importação (preview e resultado), e importação em massa via planilha XLSX/CSV com preview de 10 linhas, detecção automática de coluna de origem, lotes automáticos de 2.000 linhas sem limite máximo, barra de progresso, suporte a múltiplas origens por contato e resumo detalhado do resultado da importação — criados, já existentes, inválidos e duplicados na planilha), **configurações do workspace** (empresa, membros, perfil do usuário com avatar, MFA na UI conforme `settings`), **RBAC**, **painel super-admin** (com aviso/contador de expiração de sessão também no `/admin`; mapeamento 1 workspace : N instâncias WhatsApp via `wa_tenant_id` em `/admin/workspaces`, ver `.sdds/decisions/ADR-005-admin-wa-tenant-mapping.md`), camadas de **rate limit**, **auditoria** e **headers de segurança** descritas em `docs/security.md`. **Integração WhatsApp (aba "Integrações" em `/settings`)**: lista as instâncias do workspace, status de conexão ao vivo, QR Code de pareamento (renderizado localmente a partir do código de pareamento do provedor — nunca enviado a serviços externos), reiniciar/desconectar e edição do perfil do número (nome/descrição/foto) + edição das configurações de privacidade (visibilidade, confirmações de leitura, mensagens temporárias, listas de exceção — o provedor Z-API não expõe leitura do estado atual já configurado no número, então o painel reflete o que for definido por aqui), consumindo o backend WhatsApp como BFF (Server Actions repassam o JWT do usuário; `WA_BACKEND_URL`), com gate por papel via `settings` e auditoria — ver `.sdds/decisions/ADR-006-bff-wa-backend-management.md`. **Autorização cross-service (ADR-007)**: Custom Access Token Hook em produção carimba o claim `authz` no JWT; backend WA Fase 1 valida permissões via claim (validado fim-a-fim 2026-06-14).

Ainda em estágio inicial (placeholders ou não iniciado): **Kanban**, **inbox/chat WhatsApp**, webhooks Z-API, edição de **perfil/privacidade** do número WhatsApp (v2.3), **analytics** avançado — ver `.claude/checklist.md`.
